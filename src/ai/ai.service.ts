...
export class AiService {
  ...
  // ── CORE: Analyze job vs user profile ─────────────────────────────────
  async analyzeJobMatch(
    jobDescription: string,
    userProfile: {
      skills: Array<{ name: string; level: string; yearsExp?: number }>;
      yearsExp: number;
      headline: string;
      summary: string;
...
Be honest and specific. Return ONLY the JSON object.`;
    ...
    const result = JSON.parse(response.choices[0].message.content!) as JobAnalysisResult;
    this.logger.log(`✅ Analysis complete. Score: ${result.overallScore}`);
    return result;
  }

  // ── CORE: Tailor resume for a specific job ────────────────────────────
  async tailorResume(
    baseResume: any,
    jobDescription: string,
    jobTitle: string,
...
Return ONLY the JSON object.`;
    ...
    const result = JSON.parse(response.choices[0].message.content!) as TailoredResumeResult;
